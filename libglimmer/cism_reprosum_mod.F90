...
!                    algorithm)
!
! William Lipscomb, Jan. 2026:
! Renamed this module to cism_reprosum_mod. Modified as follows to build
!  without using CESM shared code:
! * Replaced some use statements
! * Changed 'shr' to 'cism' to avoid name conflicts with shared code
! * Output directed to iulog (declared in glimmer_paramets)
! * Added some optional debugging statements
...
!- use statements -------------------------------------------------------
!------------------------------------------------------------------------
   !WHL - Use CISM modules instead of shared modules
!#if ( defined noI8 )
...
!------------------------------------------------------------------------
   ...
   interface
      subroutine cism_reprosumx86_fix_start(arg) bind(c)
        use iso_c_binding
        integer, intent(out) :: arg
      end subroutine cism_reprosumx86_fix_start
   end interface

   interface
      subroutine cism_reprosumx86_fix_end(arg) bind(c)
        use iso_c_binding
        integer, intent(in) :: arg
      end subroutine cism_reprosumx86_fix_end
   end interface
#endif
...
! defining i8 to be shr_kind_i4 instead of shr_kind_i8.
     ! !WHL - The CISM types are declared in glimmer_global.
...
!                                     'large for integer vector algorithm' )
                  write(iulog,*) 'repro_sum failed: number of summands too '// &
                                 'large for integer vector algorithm'
                  call mpi_abort(MPI_COMM_WORLD, 1001, ierr)
                  ! end WHL mod
               endif
