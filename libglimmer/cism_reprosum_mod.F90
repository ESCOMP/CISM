!WHL, Nov. 2025: Adapted from shr_reprosum_mod.F90, part of CESM shared code
!                Changed 'shr' to 'cism' to avoid name conflicts with shared code
  
module cism_reprosum_mod
!-----------------------------------------------------------------------
!
! Purpose:
...
!!#endif
   !WHL - Use CISM modules instead of shared modules if possible
...
!!                                    !  differences against the tolerance
      ...
      cism_reprosum_tolExceeded      ! utility function to check relative
                                     !  differences against the tolerance

!-----------------------------------------------------------------------
! Public data ----------------------------------------------------------
!-----------------------------------------------------------------------
   logical, public     :: cism_reprosum_recompute = .false.
...
!------------------------------Arguments--------------------------------
      ...
      ! flag indicating whether this process should output log messages
...
!-----------------------------------------------------------------------
      ...
      if ( present(repro_sum_master) ) then
         ...
         master = .false.  !WHL master = main_task?
      endif
...
!!         logunit = s_logunit
         logunit = iulog   ! CISM default
      endif

      if (.not. firstcall) then
!!         call shr_sys_abort('shr_reprosum_setopts ERROR: multiple calls')
         write(logunit,*) 'cism_reprosum_setopts: ERROR can only be called once'
      ...
      if ( present(repro_sum_recompute_in) ) then
         cism_reprosum_recompute = repro_sum_recompute_in
      endif
      if (master) then
         ...
         if (cism_reprosum_reldiffmax >= 0._r8) then
            write(logunit,*) '                    ',&
              'with a maximum relative error tolerance of ', &
               cism_reprosum_reldiffmax
            if (cism_reprosum_recompute) then
               write(logunit,*) '                    ',&
   ...
   end subroutine cism_reprosum_setopts
...
! returned.
!
! If requested (by setting cism_reprosum_reldiffmax >= 0.0 and passing in
...
#ifdef TIMING
         call t_startf('cism_reprosum_ddpdd')
#endif
         call cism_reprosum_ddpdd(arr, arr_gsum, nsummands, dsummands, &
                                  nflds, mpi_comm)
         repro_sum_fast = 1
#ifdef TIMING
         call t_stopf('cism_reprosum_ddpdd')
#endif
      else
#ifdef TIMING
         call t_startf('cism_reprosum_int')
#endif
...
! performance for loopb in cism_reprosum_int
...
! each thread, calculated in cism_reprosum_int, is postprocessed so that
...
!!                                     'large for fixed precision algorithm' )
                  write(iulog,*) &
                       'repro_sum failed: number of summands too large for fixed precision algorithm'
                  call mpi_abort(MPI_COMM_WORLD, 1001, ierr)
               endif
...
#ifdef TIMING
         call t_stopf('cism_reprosum_int')
#endif
      endif
...
!  "normal" after level 1 calculation)
      ...
      !WHLmod - initialized the gsum also.
      !         This avoids having some garbage (lots of nonzero values) in the vector returned from mpi_reduce_sum.
      !         (Though I don't think the nonzero values do any harm
      i8_arr_gsum_level(:) = 0_i8
      !end WHLmod
...
!
   ...
   logical function cism_reprosum_tolExceeded (name, nflds, master, &
                                              logunit, rel_diff    )
!----------------------------------------------------------------------
!
! Purpose:
...
!-----------------------------------------------------------------------
!
      cism_reprosum_tolExceeded = .false.
      if (cism_reprosum_reldiffmax < 0.0_r8) return
...
!!         llogunit = s_logunit
      ...
      max_rel_diff_idx = 0
      do ifld=1,nflds
         if (rel_diff(1,ifld) > cism_reprosum_reldiffmax) then
      ...
      if (exceeds_limit > 0) then
         ...
         cism_reprosum_tolExceeded = .true.
       endif


    end function cism_reprosum_tolExceeded

!
!========================================================================
!

   subroutine cism_reprosum_ddpdd (arr, arr_gsum, nsummands, dsummands,  &
                                  nflds, mpi_comm                       )
!----------------------------------------------------------------------
!
! Purpose:
...
!-----------------------------------------------------------------------
!
      !WHL - This is a C routine in the CESM shared code: shr_reprosumx86.c
      !      Commented out the call.
      !      Not sure whether this breaks the algorithm. 
!!      call cism_reprosumx86_fix_start (old_cw)
      ...
      !WHL - commented out; see comment above
!!      call cism_reprosumx86_fix_end (old_cw)

   end subroutine cism_reprosum_ddpdd
!
!-----------------------------------------------------------------------
!
