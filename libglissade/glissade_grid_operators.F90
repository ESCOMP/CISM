...
module glissade_grid_operators
    ...
    public :: glissade_stagger, glissade_unstagger,    &
              glissade_gradient,   &
              !TODO - Remove the next 4 subroutines
              glissade_centered_gradient, glissade_upstream_gradient,    &
              glissade_gradient_at_edges, glissade_surface_elevation_gradient,  &
              glissade_surface_elevation_gradient_new,  &
...
!TODO - Remove this subroutine after verifying BFB with new subroutine
...
!TODO - Remove this subroutine
  ...
  subroutine glissade_upstream_gradient(nx,           ny,        &
    ...
    !  various values of ho_gradient_margin. 
    !----------------------------------------------------------------
    ...
    integer, intent(in), optional ::    &
       ho_gradient_margin_in    ! 0: use all values when computing gradient (including zeroes where ice is absent)
    ...
    integer, intent(in), optional ::    &
       ho_gradient_in           ! = 1 for 1st order, 2 for 2nd order

    integer, dimension(nx,ny), intent(in), optional ::        &
       floating_mask,        &  ! = 1 where ice is present and floating, else = 0
       land_mask                ! = 1 for land cells, else = 0
                                ! floating and land masks required for ho_gradient_margin = HO_GRADIENT_MARGIN_GROUNDED_ICE
    ...
    integer :: ho_gradient, ho_gradient_margin
    integer :: i, j
    integer :: summask
    real(dp) :: sum1, sum2
    ...
    if (present(ho_gradient_in)) then
       ho_gradient = ho_gradient_in
    else
       ho_gradient = 2   ! default to second-order
    endif

    if (present(ho_gradient_margin_in)) then
       ho_gradient_margin = ho_gradient_margin_in
    else
       ho_gradient_margin = HO_GRADIENT_MARGIN_GROUNDED_ICE
    endif
    ...
    elseif (ho_gradient_margin == HO_GRADIENT_MARGIN_GROUNDED_ICE) then
    ...
    if (ho_gradient == HO_GRADIENT_UPSTREAM1) then   ! first-order accurate
    ...
    elseif (ho_gradient == HO_GRADIENT_UPSTREAM2) then   ! second-order accurate
...
!TODO - Remove this subroutine after verifying same answers with new subroutine
...
!****************************************************************************
  ...
  subroutine glissade_surface_elevation_gradient_new(nx,          ny,         &
                                                 dx,          dy,         &
                                                 active_ice_mask,         &
                                                 land_mask,               &
                                                 usrf,        thck,       &
    ...
    !
    ! This option used to be the default. However, it can overestimate driving forces at grounded marine
    !  margins where lateral spreading dominates the driving force.
    !
    ! HO_GRADIENT_MARGIN_ICE_ONLY = 2: Only values in ice-covered cells (i.e., cells with thck > thklim)
    !  are used to compute gradients.  If one or both adjacent cells is ice-free, the edge is masked out.
    !  This option works well at shelf margins but less well for land margins (e.g., the Halfar test case).
    ...
    ! (2) One cell is ice-covered and lies above ice-free land.
    !
    ! Method 3 sets the gradient to zero at edges where
    ! (1) An ice-covered cell (grounded or floating) lies above ice-free ocean.
    !     Note: Inactive calving-front cells are treated as ice-free ocean.
    ! (2) An ice-covered land cell lies below an ice-free land cell (i.e., a nunatak).
    !
    ...
    !  margins the gradient is zero.
    !
    !TODO - Add notes on ho_gradient options
    !----------------------------------------------------------------
    ...
    elseif (ho_gradient_margin == HO_GRADIENT_MARGIN_ICE_OVER_LAND) then
...
!!          do i = 1, nx-1
  ...
  end subroutine glissade_surface_elevation_gradient_new
