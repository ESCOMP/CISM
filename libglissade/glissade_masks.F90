...
!****************************************************************************
  ...
  subroutine glissade_marine_grounded_mask(&
       nx,                     ny,                   &
       ice_mask,               floating_mask,        &
       land_mask,                                    &
       marine_grounded_mask)

    ! Compute a mask to identify marine grounded cells.
    ! These are defined as cells with ice grounded below sea-level.
    ...
    integer, dimension(nx,ny), intent(out) ::  &
         marine_grounded_mask     ! = 1 if ice is grounded, marine-based 
    ...
    marine_grounded_mask(:,:) = 0

    do j = 1, ny
       do i = 1, nx
          if (ice_mask(i,j) == 1 .and. land_mask(i,j) == 0 .and. floating_mask(i,j) == 0) then ! grounded marine-based ice
             marine_grounded_mask(i,j) = 1
          endif  ! grounded marine-based ice
       enddo  ! i
    enddo   ! j
...
!****************************************************************************
  ...
  subroutine glissade_grounded_calving_front_mask(&
       nx,                     ny,                   &
       which_ho_calving_front,                       &
       parallel,                                     &
       thck,                   topg,                 &
    ...
    ! Compute a grounded calving front mask for calving at a typical Greenland marine margin,
    !   effective calving_front thickness, and related fields.
    ! If using the subgrid calving front scheme, then compute partial CF cells.
    ! The grounded calving front is a floating cell connected to a grounded cell and open ocean. 
    ...
    integer, dimension(nx,ny), intent(out) ::  &
         grounded_calving_front_mask       ! = 1 if ice is floating and borders at least one ocean cell, else = 0
    ...
    real(dp) :: &
         max_neighbor_thck,        & ! max thickness (m) of the four edge neighbors
         min_neighbor_thck,        & ! min thickness (m) of the four edge neighbors
         distance,                 & ! distance between adjacent cell centers
         dthck_dx                    ! dH/dx between adjacent cells near the CF

    integer, dimension(nx,ny) ::  &
         marine_grounded_mask,    &  ! = 1 if ice is grounded, marine-based 
         grounded_mask,           &  ! = 1 if ice is grounded
         marine_cliff_mask           ! = 1 if ice is a grounded marine cliff
    ...
    ! TODO use calving%marine_cliff_mask here
    marine_cliff_mask(:,:) = 0
    ...
    grounded_calving_front_mask(:,:) = 0

    ! The grounded calving front consists of marine cliffs
    do j = 2, ny-1
       do i = 2, nx-1
          if (marine_cliff_mask(i,j) == 1) then
             grounded_calving_front_mask(i,j) = 1
          endif
       enddo
    enddo

    call parallel_halo(grounded_calving_front_mask, parallel)
    

    if (which_ho_calving_front == HO_CALVING_FRONT_SUBGRID) then

       ! In addition to marine cliffs, the subgrid grounded calving front also includes
       !   floating cells that border marine grounded cells 
       do j = 2, ny-1
          do i = 2, nx-1
             ! Identify subgrid calving front cells: floating cells that border marine
             !   grounded cells (which makes those last-marine-grounded cells) 
             if (floating_mask(i,j) == 1) then
                if (marine_grounded_mask(i-1,j) == 1 .or. marine_grounded_mask(i+1,j) == 1 .or. &
                     marine_grounded_mask(i,j-1) == 1 .or. marine_grounded_mask(i,j+1) == 1) then
                   grounded_calving_front_mask(i,j) = 1
                endif ! adjacent to marine grounded cells
             endif ! floating
          enddo
       enddo
       ...
       ! Identify full grounded cells and partial CF cells.
       ! All ice-covered cells not at the CF are full cells.
       ! For CF cells, compute the max thickness of interior neighbors as effective thickness

       !thck_submersion = max(eus - topg, 0.0d0) ! may be needed for additional constraints
       thck_flotation = max(-(rhoo/rhoi) * (topg - eus), 0.0d0)
       capped_thck = min(thck, thck_flotation)
       ...
       do j = 2, ny-1
          do i = 2, nx-1
             if (ice_mask(i,j) == 1) then
                if (grounded_calving_front_mask(i,j) == 1) then
                   !! max of marine grounded neighbors
                   !max_neighbor_thck = max(&
                   !     marine_grounded_mask(i-1,j)*capped_thck(i-1,j), marine_grounded_mask(i+1,j)*capped_thck(i+1,j), &
                   !     marine_grounded_mask(i,j-1)*capped_thck(i,j-1), marine_grounded_mask(i,j+1)*capped_thck(i,j+1))
                   ! max of grounded neighbors
                   max_neighbor_thck = max(&
                        grounded_mask(i-1,j)*capped_thck(i-1,j), grounded_mask(i+1,j)*capped_thck(i+1,j), &
                        grounded_mask(i,j-1)*capped_thck(i,j-1), grounded_mask(i,j+1)*capped_thck(i,j+1))
                   !! max of capped grounded neighbors limited again by local floating thickness
                   !max_neighbor_thck = max(&
                   !     grounded_mask(i-1,j)*capped_thck(i-1,j), grounded_mask(i+1,j)*capped_thck(i+1,j), &
                   !     grounded_mask(i,j-1)*capped_thck(i,j-1), grounded_mask(i,j+1)*capped_thck(i,j+1))
                   !max_neighbor_thck = min(max_neighbor_thck,capped_thck(i,j))
                   ! TODO: check if these should be further constrained, e.g. to last grounded cells only?
                   !       why max and not min or mean?
                   !       further constrain to limit effective surface elevation by local bed?
                   partial_cf_mask(i,j) = 1
                   thck_effective(i,j) = max_neighbor_thck
                else   ! not a CF cell; thck_effective = thck
                   full_mask(i,j) = 1
                endif  ! grounded_calving_front_mask
             endif   ! ice_mask
          enddo   ! i
       enddo   ! j

       ! Limit thck_effective in some way?
       !where (grounded_calving_front_mask == 1)
       !   thck_effective = min(thck_effective, thck_flotation)
       !endwhere
  ...
  end subroutine glissade_grounded_calving_front_mask

!****************************************************************************

  subroutine glissade_secondary_floating_mask(&
       nx,                     ny,                   &
       floating_mask,          marine_grounded_mask, &
       secondary_floating_mask)

    ! Compute a mask to identify 'secondary' floating cells.
    ! These are defined as floating cells with no grounded marine ice as neighbors
    ! The output can be used to identify cells for a secondary float kill, removing
    ! floating ice that cannot become a subgrid calving front.
    ...
    integer, dimension(nx,ny), intent(in) ::  &
         floating_mask,         & ! = 1 if thck > thklim and ice is floating, else = 0
         marine_grounded_mask     ! = 1 if cell is a marine grounded

    integer, dimension(nx,ny), intent(out) ::  &
         secondary_floating_mask        ! = 1 if ice is floating and not neighboring a marine grounded cell
    ...
    secondary_floating_mask(:,:) = 0

    do j = 2, ny-1
       do i = 2, nx-1
          if (floating_mask(i,j) == 1) then
             secondary_floating_mask(i,j) = 1
             if (marine_grounded_mask(i-1,j) == 1 .or. marine_grounded_mask(i+1,j) == 1 .or. &
                 marine_grounded_mask(i,j-1) == 1 .or. marine_grounded_mask(i,j+1) == 1) then
                secondary_floating_mask(i,j) = 0
             endif ! marine neighbor
          endif  ! floating
       enddo  ! i
    enddo   ! j

  end subroutine glissade_secondary_floating_mask
