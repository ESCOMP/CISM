...
# Handle other custom substitutions
for name, value in vars(subs).items():
    # Skip standard substitutions we've already handled
    if name in ["project", "copyright", "author", "version", "release", "version_label"]:
        continue
    # Skip members of subs that aren't intended as substitutions
    if name.startswith("_") or not isinstance(value, str):
        continue
    # Append custom substitutions to rst_epilog
...
# Make substitutions from project's substitutions.py available in Markdown files
myst_substitutions = {
    name: value
    for name, value in vars(subs).items()
