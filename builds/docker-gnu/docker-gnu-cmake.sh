...
\
  -D CISM_EXTRA_LIBS:STRING="-L/usr/lib64 -llapack" \
  -D CMAKE_EXE_LINKER_FLAGS="-Wl,--copy-dt-needed-entries" \
\
  -D CMAKE_Fortran_FLAGS:STRING="-g -O2 -ffree-line-length-none -fallow-argument-mismatch" \
  -D CMAKE_C_FLAGS:STRING="-g -O2" \
  -D CMAKE_CXX_FLAGS:STRING="-g -O2" \
  "${cism_top}"
