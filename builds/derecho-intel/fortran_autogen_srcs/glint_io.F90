  ...
  subroutine glint_io_write(outfile,data)
    ...
    if (status .eq. nf90_noerr) then
       status = distributed_put_var(NCO%id, varid, &
            data%ablt, parallel, (/1,1,outfile%timecounter/))
       call nc_errorhandle(__FILE__,__LINE__,status)
    end if
    ...
    if (status .eq. nf90_noerr) then
       status = distributed_put_var(NCO%id, varid, &
            data%arng, parallel, (/1,1,outfile%timecounter/))
       call nc_errorhandle(__FILE__,__LINE__,status)
    end if
    ...
    if (status .eq. nf90_noerr) then
       status = distributed_put_var(NCO%id, varid, &
            data%global_orog, parallel, (/1,1,outfile%timecounter/))
       call nc_errorhandle(__FILE__,__LINE__,status)
    end if
    ...
    if (status .eq. nf90_noerr) then
       status = distributed_put_var(NCO%id, varid, &
            data%hflx_tavg, parallel, (/1,1,outfile%timecounter/))
       call nc_errorhandle(__FILE__,__LINE__,status)
    end if
    ...
    if (status .eq. nf90_noerr) then
       status = distributed_put_var(NCO%id, varid, &
            data%downs%lmask, parallel, (/1,1,outfile%timecounter/))
       call nc_errorhandle(__FILE__,__LINE__,status)
    end if
    ...
    if (status .eq. nf90_noerr) then
       status = distributed_put_var(NCO%id, varid, &
            data%local_orog, parallel, (/1,1,outfile%timecounter/))
       call nc_errorhandle(__FILE__,__LINE__,status)
    end if
    ...
    if (status .eq. nf90_noerr) then
       status = distributed_put_var(NCO%id, varid, &
            data%out_mask, parallel, (/1,1,outfile%timecounter/))
       call nc_errorhandle(__FILE__,__LINE__,status)
    end if
    ...
    if (status .eq. nf90_noerr) then
       status = distributed_put_var(NCO%id, varid, &
            data%prcp, parallel, (/1,1,outfile%timecounter/))
       call nc_errorhandle(__FILE__,__LINE__,status)
    end if
    ...
    if (status .eq. nf90_noerr) then
       status = distributed_put_var(NCO%id, varid, &
            data%rofi_tavg, parallel, (/1,1,outfile%timecounter/))
       call nc_errorhandle(__FILE__,__LINE__,status)
    end if
    ...
    if (status .eq. nf90_noerr) then
       status = distributed_put_var(NCO%id, varid, &
            data%rofl_tavg, parallel, (/1,1,outfile%timecounter/))
       call nc_errorhandle(__FILE__,__LINE__,status)
    end if
    ...
    if (status .eq. nf90_noerr) then
       status = distributed_put_var(NCO%id, varid, &
            data%siced, parallel, (/1,1,outfile%timecounter/))
       call nc_errorhandle(__FILE__,__LINE__,status)
    end if
    ...
    if (status .eq. nf90_noerr) then
       status = distributed_put_var(NCO%id, varid, &
            data%snowd, parallel, (/1,1,outfile%timecounter/))
       call nc_errorhandle(__FILE__,__LINE__,status)
    end if
  ...
  subroutine glint_io_read(infile,data)
    ...
    if (is_enabled(data%hflx_tavg)) then
       ...
       status = distributed_get_var(NCI%id, varid, &
            data%hflx_tavg, parallel, (/1,1,infile%current_time/))
       call nc_errorhandle(__FILE__,__LINE__,status)
    ...
    if (is_enabled(data%out_mask)) then
       ...
       status = distributed_get_var(NCI%id, varid, &
            data%out_mask, parallel, (/1,1,infile%current_time/))
       call nc_errorhandle(__FILE__,__LINE__,status)
       status = parallel_get_att(NCI%id, varid,'scale_factor',scaling_factor)
       if (status.ne.NF90_NOERR) then
          scaling_factor = 1.0d0
    ...
    if (is_enabled(data%rofi_tavg)) then
       ...
       status = distributed_get_var(NCI%id, varid, &
            data%rofi_tavg, parallel, (/1,1,infile%current_time/))
       call nc_errorhandle(__FILE__,__LINE__,status)
       status = parallel_get_att(NCI%id, varid,'scale_factor',scaling_factor)
       if (status.ne.NF90_NOERR) then
          scaling_factor = 1.0d0
    ...
    if (is_enabled(data%rofl_tavg)) then
       ...
       status = distributed_get_var(NCI%id, varid, &
            data%rofl_tavg, parallel, (/1,1,infile%current_time/))
       call nc_errorhandle(__FILE__,__LINE__,status)
       status = parallel_get_att(NCI%id, varid,'scale_factor',scaling_factor)
       if (status.ne.NF90_NOERR) then
          scaling_factor = 1.0d0
    ...
    if (is_enabled(data%siced)) then
       ...
       status = distributed_get_var(NCI%id, varid, &
            data%siced, parallel, (/1,1,infile%current_time/))
       call nc_errorhandle(__FILE__,__LINE__,status)
       status = parallel_get_att(NCI%id, varid,'scale_factor',scaling_factor)
       if (status.ne.NF90_NOERR) then
          scaling_factor = 1.0d0
    ...
    if (is_enabled(data%snowd)) then
       ...
       status = distributed_get_var(NCI%id, varid, &
            data%snowd, parallel, (/1,1,infile%current_time/))
       call nc_errorhandle(__FILE__,__LINE__,status)
       status = parallel_get_att(NCI%id, varid,'scale_factor',scaling_factor)
       if (status.ne.NF90_NOERR) then
          scaling_factor = 1.0d0
