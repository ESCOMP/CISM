  ...
  subroutine glint_mbal_io_write(outfile,data)
    ...
    if (status .eq. nf90_noerr) then
       status = distributed_put_var(NCO%id, varid, &
            data%mbal_accum%ablt, parallel, (/1,1,outfile%timecounter/))
       call nc_errorhandle(__FILE__,__LINE__,status)
    end if
    ...
    if (status .eq. nf90_noerr) then
       status = distributed_put_var(NCO%id, varid, &
            data%mbal_accum%acab, parallel, (/1,1,outfile%timecounter/))
       call nc_errorhandle(__FILE__,__LINE__,status)
    end if
    ...
    if (status .eq. nf90_noerr) then
       status = distributed_put_var(NCO%id, varid, &
            data%mbal_accum%artm, parallel, (/1,1,outfile%timecounter/))
       call nc_errorhandle(__FILE__,__LINE__,status)
    end if
    ...
    if (status .eq. nf90_noerr) then
       status = distributed_put_var(NCO%id, varid, &
            data%mbal_accum%humidity, parallel, (/1,1,outfile%timecounter/))
       call nc_errorhandle(__FILE__,__LINE__,status)
    end if
    ...
    if (status .eq. nf90_noerr) then
       status = distributed_put_var(NCO%id, varid, &
            data%mbal_accum%lwdown, parallel, (/1,1,outfile%timecounter/))
       call nc_errorhandle(__FILE__,__LINE__,status)
    end if
    ...
    if (status .eq. nf90_noerr) then
       status = distributed_put_var(NCO%id, varid, &
            data%mbal_accum%prcp, parallel, (/1,1,outfile%timecounter/))
       call nc_errorhandle(__FILE__,__LINE__,status)
    end if
    ...
    if (status .eq. nf90_noerr) then
       status = distributed_put_var(NCO%id, varid, &
            data%mbal_accum%psurf, parallel, (/1,1,outfile%timecounter/))
       call nc_errorhandle(__FILE__,__LINE__,status)
    end if
    ...
    if (status .eq. nf90_noerr) then
       status = distributed_put_var(NCO%id, varid, &
            data%mbal_accum%siced, parallel, (/1,1,outfile%timecounter/))
       call nc_errorhandle(__FILE__,__LINE__,status)
    end if
    ...
    if (status .eq. nf90_noerr) then
       status = distributed_put_var(NCO%id, varid, &
            data%mbal_accum%snowd, parallel, (/1,1,outfile%timecounter/))
       call nc_errorhandle(__FILE__,__LINE__,status)
    end if
    ...
    if (status .eq. nf90_noerr) then
       status = distributed_put_var(NCO%id, varid, &
            data%mbal_accum%swdown, parallel, (/1,1,outfile%timecounter/))
       call nc_errorhandle(__FILE__,__LINE__,status)
    end if
    ...
    if (status .eq. nf90_noerr) then
       status = distributed_put_var(NCO%id, varid, &
            data%mbal_accum%xwind, parallel, (/1,1,outfile%timecounter/))
       call nc_errorhandle(__FILE__,__LINE__,status)
    end if
    ...
    if (status .eq. nf90_noerr) then
       status = distributed_put_var(NCO%id, varid, &
            data%mbal_accum%ywind, parallel, (/1,1,outfile%timecounter/))
       call nc_errorhandle(__FILE__,__LINE__,status)
    end if
